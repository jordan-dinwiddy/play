...
public class ArrayBasedPriorityQueue<T> implements Queue<T> {
	...
	public void enqueue(T item, Priority p) {
		...
		heapUp(data.size() - 1);	// Move it up the heap
	}
	...
	private void heapDown(int ix) {
		...
		int largestChildIx = leftChildIx;
		QueueEntry<T> largestChildEntry = data.get(largestChildIx);

		if(rightChildIx < data.size()) {
			// If we have a right child and the right is greater than the left...
			QueueEntry<T> rightChildEntry = data.get(rightChildIx);

			if(rightChildEntry.priority.nValue > largestChildEntry.priority.nValue) {
				largestChildIx = rightChildIx;
				largestChildEntry = rightChildEntry;
			}
		}

		if(entry.priority.nValue < largestChildEntry.priority.nValue) {
			swap(ix, largestChildIx);
			heapDown(largestChildIx);
		}
	}
